...
from .base import MessageBroker, MessageConfig, Message
...
class RedisMessageBroker(MessageBroker):
    ...
    def __init__(self, config: RedisMessageConfig):
        ...
        self._stream_consumers: Dict[str, str] = {}  # topic -> consumer_group
        self._stream_consumer_tasks: Dict[str, asyncio.Task] = {}
    ...
    async def subscribe_stream(
        ...
        try:
            ...
            # 핸들러 저장
            self._message_handlers[topic] = handler
            self._stream_consumers[topic] = consumer_group
            
            # 스트림 소비 태스크 시작
            task = asyncio.create_task(
                self._consume_stream(topic, stream_name, consumer_group, consumer_name)
            )
            self._stream_consumer_tasks[topic] = task
    ...
    async def unsubscribe_stream(self, topic: str) -> Result[None, str]:
        ...
        try:
            # 소비 태스크 중지
            if topic in self._stream_consumer_tasks:
                task = self._stream_consumer_tasks.pop(topic)
                task.cancel()
                try:
                    await task
                except asyncio.CancelledError:
            ...
            # 정보 제거
            self._message_handlers.pop(topic, None)
            self._stream_consumers.pop(topic, None)
                        ...
                        for stream, stream_messages in messages:
                            for message_id, fields in stream_messages:
                                await self._process_stream_message(
                                    topic, stream_name, consumer_group,
                                    message_id, fields
                                )
    ...
    async def get_consumer_group_info(self, topic: str) -> Result[List[Dict[str, Any]], str]:
