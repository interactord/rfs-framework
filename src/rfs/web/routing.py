...
class Route:
    ...
    def __post_init__(self):
        ...
        if self.name is None:
            self.name = f"{self.method.lower()}_{self.path.replace('/', '_').strip('_')}"
...
class RouteGroup:
    ...
    def add_route(self, 
        ...
        kwargs['middleware'] = self.middleware + route_middleware
        ...
        try:
            ...
            for route in self.router.get_routes():
                ...
                if route.method == HTTPMethod.GET:
                    api_router.get(route.path, 
                                  summary=route.summary, 
                                  description=route.description,
                                  tags=route.tags)(route.handler)
                elif route.method == HTTPMethod.POST:
                    api_router.post(route.path, 
                                   summary=route.summary, 
                                   description=route.description,
                                   tags=route.tags)(route.handler)
                elif route.method == HTTPMethod.PUT:
                    api_router.put(route.path, 
                                  summary=route.summary, 
                                  description=route.description,
                                  tags=route.tags)(route.handler)
                elif route.method == HTTPMethod.DELETE:
                    api_router.delete(route.path, 
                                     summary=route.summary, 
                                     description=route.description,
                                     tags=route.tags)(route.handler)
                elif route.method == HTTPMethod.PATCH:
                    api_router.patch(route.path, 
                                    summary=route.summary, 
                                    description=route.description,
                                    tags=route.tags)(route.handler)
