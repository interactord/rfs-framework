...
class ComplianceStatus(Enum):
    ...
    PARTIALLY_COMPLIANT = "partially_compliant"  # 부분 준수
...
class CompliancePolicy:
    ...
    validation_frequency: str  # Cron expression
...
class ComplianceValidator:
        ...
        try:
            ...
            for control in standard_controls:
                ...
                if isinstance(result, Success):
                    control_results.append(result.value)
                    total_score += result.value.score
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            for standard in standards:
                ...
                if latest_report:
                    standard_reports[standard.value] = latest_report
            
            # 전체 컴플라이언스 점수 계산
            total_score = sum(
                report.overall_score 
                for report in standard_reports.values()
            ) / len(standard_reports) if standard_reports else 0
            ...
            return Success({
                ...
                "key_findings": unique_findings[:10],  # Top 10
    ...
    async def check_security_compliance(
        ...
        try:
            ...
            if (password_policy.get("min_length", 0) >= 12 and
                password_policy.get("complexity_required")):
                checks["password_policy"] = True
            
            # 세션 타임아웃 확인
            if security_config.get("session_timeout", 0) <= 3600:  # 1시간 이내
        ...
        for requirement in control.requirements:
            ...
            if control.category == ControlCategory.ACCESS_CONTROL:
                if "access_control" not in context:
                    findings.append(f"Missing access control implementation for: {requirement}")
                    recommendations.extend(control.remediation_steps)
                    score -= 20
            
            elif control.category == ControlCategory.DATA_PROTECTION:
                if "encryption" not in context or not context["encryption"]:
                    findings.append(f"Data protection not implemented: {requirement}")
                ...
                if "firewall" not in context:
                    findings.append(f"Network security control missing: {requirement}")
    ...
    def _generate_security_recommendations(
        ...
        if not checks["password_policy"]:
            recommendations.append("Strengthen password policy requirements")
    ...
    def _get_latest_report(
        ...
        filtered_reports = [
            ...
            if r.standard == standard and
               start_date <= r.generated_at <= end_date
        ]
    ...
    def _calculate_compliance_trends(self) -> Dict[str, Any]:
        ...
        for month in range(6):
            ...
            month_reports = [
                r for r in self.reports
                if month_start <= r.generated_at <= month_end
            ]
            
            if month_reports:
                avg_score = sum(r.overall_score for r in month_reports) / len(month_reports)
    ...
    def _identify_risk_areas(
        self,
        standard_reports: Dict[str, ComplianceReport]
    ) -> List[Dict[str, Any]]:
        """위험 영역 식별"""
        risk_areas = []
        ...
        return risk_areas[:10]  # Top 10 risk areas
