...
class RequestResponse:
    ...
    async def respond(self, request_message: Message, response_data: Any) -> Result[None, str]:
        """응답 전송"""
        try:
...
class EventBus:
    ...
    async def unsubscribe_event(self, event_type: str, handler: Callable = None) -> Result[None, str]:
        """이벤트 구독 해제"""
        try:
    ...
    async def execute(self) -> Result[Dict[str, Any], str]:
        ...
        try:
            ...
            await self._publish_saga_event("saga.completed", {
                ...
                "duration": (datetime.now() - self.context.created_at).total_seconds()
            })
            ...
            try:
                ...
                if result.is_success():
                    ...
                    if attempt < step.retries:
                        logger.warning(f"Saga 단계 재시도 ({step.name}): {attempt + 1}/{step.retries}")
                        await asyncio.sleep(1.0 * (attempt + 1))  # 지수적 백오프
                    else:
                        return result
                
            except asyncio.TimeoutError:
                if attempt < step.retries:
                    logger.warning(f"Saga 단계 타임아웃 재시도 ({step.name}): {attempt + 1}/{step.retries}")
                ...
                if attempt < step.retries:
                    logger.warning(f"Saga 단계 오류 재시도 ({step.name}): {attempt + 1}/{step.retries} - {e}")
                    await asyncio.sleep(1.0 * (attempt + 1))
                else:
...
class MessageRouter:
    ...
    def __init__(self, broker_name: str = None):
        ...
        self._routes: Dict[str, List[Callable]] = {}  # pattern -> handlers
        self._subscribers: Dict[str, Subscriber] = {}
        self._publisher = Publisher(broker_name)
    ...
    async def start_routing(self, input_topic: str) -> Result[None, str]:
        ...
        try:
            if input_topic not in self._subscribers:
                ...
                self._subscribers[input_topic] = subscriber
    ...
    async def _route_message(self, message: Message):
        ...
        try:
            ...
            if not routed:
                logger.warning(f"라우팅되지 않은 메시지: {message.topic} - {message.id}")
    ...
    def _match_pattern(self, message: Message, pattern: str) -> bool:
        ...
        try:
            ...
            # 헤더 기반 매칭 (pattern:header.key=value 형식)
            if ":" in pattern:
