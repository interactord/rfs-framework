...
class SystemMetrics:
    ...
    def get_average_usage(self, minutes: int = 10) -> Optional[ResourceUsage]:
        ...
        # 네트워크 I/O 평균 (bytes_sent, bytes_recv)
        avg_network = {
            "bytes_sent": sum(u.network_io.get("bytes_sent", 0) for u in recent) / len(recent),
            "bytes_recv": sum(u.network_io.get("bytes_recv", 0) for u in recent) / len(recent)
        }
    ...
    async def export_metrics(self, format: str = "json") -> Result[Dict[str, Any], str]:
        ...
        try:
            if format == "json":
                data = {
                    "system_info": self.metrics.system_info.__dict__,
                    "resource_history": [usage.to_dict() for usage in self.metrics.resource_history],
