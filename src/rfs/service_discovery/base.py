...
class ServiceInfo:
    ...
    name: str = ""
    endpoint: ServiceEndpoint = field(default_factory=lambda: ServiceEndpoint("localhost", 8080))
    ...
    ttl: Optional[timedelta] = timedelta(seconds=30)  # Time-To-Live
    
    # 의존성
    dependencies: List[str] = field(default_factory=list)  # 의존 서비스 이름
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ServiceInfo':
        ...
        health = ServiceHealth(
            ...
            last_check=datetime.fromisoformat(data['health']['last_check']) if data['health'].get('last_check') else None
        )
