...
class KPIThreshold:
    ...
    def evaluate(self, value: float) -> bool:
        ...
        elif self.threshold_type == ThresholdType.NOT_BETWEEN:
...
class KPI(ABC):
    ...
    def add_threshold(self, threshold: KPIThreshold) -> Result[bool, str]:
        ...
        try:
            ...
            # 우선순위 순으로 정렬 (critical > warning > normal)
            self.thresholds.sort(key=lambda t: {
...
class CountKPI(KPI):
    ...
    async def calculate(self, **kwargs) -> Result[float, str]:
        ...
        try:
            ...
            return Failure(f"Count calculation failed: {str(e)}")
...
class AverageKPI(KPI):
    ...
    async def calculate(self, **kwargs) -> Result[float, str]:
        ...
        try:
            ...
            return Failure(f"Average calculation failed: {str(e)}")


class PercentageKPI(KPI):
    """퍼센티지 KPI"""
    
    def __init__(self, kpi_id: str, name: str, numerator_query: str, 
                 denominator_query: str, **kwargs):
        super().__init__(kpi_id, name, **kwargs)
    ...
    async def calculate(self, **kwargs) -> Result[float, str]:
        ...
        try:
            ...
            # 소수점 둘째 자리까지 반올림
            percentage = float(Decimal(str(percentage)).quantize(
...
class TrendKPI(KPI):
    ...
    def __init__(self, kpi_id: str, name: str, query: str, 
                 time_column: str, value_column: str, **kwargs):
        super().__init__(kpi_id, name, **kwargs)
        self.query = query
    ...
    async def calculate(self, **kwargs) -> Result[float, str]:
        ...
        try:
            ...
            # 시간순 정렬
            try:
                data.sort(key=lambda x: datetime.fromisoformat(
                    x[self.time_column].replace('Z', '+00:00')
                ) if isinstance(x[self.time_column], str) else x[self.time_column])
            except Exception:
                # 정렬 실패시 원본 순서 유지
                pass
...
class KPICalculator:
    ...
    async def calculate_kpi(self, kpi_id: str, use_cache: bool = True, **kwargs) -> Result[KPIValue, str]:
        """KPI 계산"""
        if kpi_id not in self._kpis:
            return Failure(f"KPI not found: {kpi_id}")
        ...
        if result.is_success():
            # 캐시 업데이트
            self._calculation_cache[kpi_id] = result.unwrap()
        
        return result
    
    async def calculate_all_kpis(self, use_cache: bool = True, **kwargs) -> Dict[str, Result[KPIValue, str]]:
        """모든 KPI 계산"""
        results = {}
    ...
    def get_kpi_summary(self) -> Dict[str, Any]:
        ...
        for kpi in self._kpis.values():
            ...
            summary["by_type"][kpi_type] = summary["by_type"].get(kpi_type, 0) + 1
            ...
            if current_value:
                ...
                if (summary["last_updated"] is None or 
                    current_value.timestamp > summary["last_updated"]):
                    summary["last_updated"] = current_value.timestamp
    ...
    def get_status_summary(self) -> Dict[str, int]:
        ...
        for kpi_id in self.kpi_ids:
            ...
            if kpi_result.is_success():
                ...
                current_value = kpi.get_current_value()
                if current_value:
                    summary[current_value.status.value] += 1
        ...
        for config in kpi_configs:
            ...
            for target_config in config.get("targets", []):
                target = KPITarget(
                    ...
                    target_date=datetime.fromisoformat(target_config["date"]) if target_config.get("date") else None,
...
# 헬퍼 함수들
def create_count_kpi(kpi_id: str, name: str, query: str, data_source: DataSource) -> CountKPI:
    """카운트 KPI 생성"""
    return CountKPI(kpi_id, name, query, data_source=data_source)


def create_average_kpi(kpi_id: str, name: str, query: str, column: str, data_source: DataSource) -> AverageKPI:
    """평균 KPI 생성"""
    return AverageKPI(kpi_id, name, query, column, data_source=data_source)


def create_percentage_kpi(kpi_id: str, name: str, num_query: str, den_query: str, data_source: DataSource) -> PercentageKPI:
    """퍼센티지 KPI 생성"""
    return PercentageKPI(kpi_id, name, num_query, den_query, data_source=data_source)


def create_threshold(threshold_id: str, name: str, threshold_type: ThresholdType, 
                    values: List[float], status: KPIStatus, message: str = "") -> KPIThreshold:
    """임계값 생성"""
