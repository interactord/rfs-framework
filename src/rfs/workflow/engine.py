...
class WorkflowContext:
    ...
    def get_step_output(self, step_id: str, key: str = None, default: Any = None) -> Any:
        """스텝 출력 조회"""
...
class WorkflowEvent(Event):
    ...
    instance_id: str
    workflow_id: str
    event_type: str  # started, completed, failed, step_completed, etc.
...
class WorkflowEngine:
    ...
    async def start_workflow(self, workflow_definition: 'WorkflowDefinition',
                           input_data: Optional[Dict[str, Any]] = None) -> Result[WorkflowInstance, str]:
        """워크플로우 시작"""
    ...
    async def list_instances(self, workflow_id: Optional[str] = None,
    ...
    async def _execute_step(self, instance: WorkflowInstance, step: 'WorkflowStep') -> Result[Any, str]:
        """워크플로우 스텝 실행"""
        ...
        instance.step_executions.append(step_execution)
        ...
        try:
            ...
            if result.is_success():
                ...
                # 출력 데이터를 컨텍스트에 저장
                instance.context.step_outputs[step.id] = output_data
                ...
                await logger.log_info(f"스텝 실행 완료: {step.name} ({step_execution.duration:.3f}s)")
    ...
    def _prepare_step_input(self, instance: WorkflowInstance, step: 'WorkflowStep') -> Dict[str, Any]:
        ...
        if hasattr(step, 'input_mapping') and step.input_mapping:
            for output_key, input_expr in step.input_mapping.items():
                # 간단한 표현식 처리 (예: ${variables.user_id}, ${steps.step1.result})
                if input_expr.startswith("${") and input_expr.endswith("}"):
                    expr = input_expr[2:-1]  # ${}를 제거
                    
                    if expr.startswith("variables."):
                        var_name = expr[10:]  # "variables." 제거
                        input_data[output_key] = instance.context.get_variable(var_name)
                    elif expr.startswith("steps."):
                        parts = expr[6:].split(".", 1)  # "steps." 제거 후 분할
                        if len(parts) == 2:
                            step_id, key = parts
                            input_data[output_key] = instance.context.get_step_output(step_id, key)
                        else:
                            step_id = parts[0]
                            input_data[output_key] = instance.context.get_step_output(step_id)
                else:
                    input_data[output_key] = input_expr
...
# 편의 함수들
async def start_workflow(workflow_definition: 'WorkflowDefinition',
                        input_data: Optional[Dict[str, Any]] = None) -> Result[WorkflowInstance, str]:
    """워크플로우 시작"""
...
async def execute_workflow(workflow_definition: 'WorkflowDefinition',
                          input_data: Optional[Dict[str, Any]] = None) -> Result[WorkflowInstance, str]:
    """워크플로우 실행 (시작과 동일)"""
