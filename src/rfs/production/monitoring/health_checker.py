...
class CheckType(Enum):
    """체크 유형"""
    HTTP = "http"             # HTTP 엔드포인트
    TCP = "tcp"               # TCP 포트
    DATABASE = "database"     # 데이터베이스
    SERVICE = "service"       # 외부 서비스
    CUSTOM = "custom"         # 커스텀 체크
    RESOURCE = "resource"     # 시스템 리소스
...
class HealthCheckConfig:
    ...
    unhealthy_threshold: int = 3  # 연속 실패 횟수
    healthy_threshold: int = 2    # 연속 성공 횟수
...
class EndpointCheck:
    ...
    async def initialize(self) -> None:
        ...
        if not self.session:
            timeout = aiohttp.ClientTimeout(
                total=self.config.config.get('timeout', 10)
            )
            ...
            if method == 'GET':
                async with self.session.get(url, headers=headers) as response:
                    ...
                    return await self._evaluate_response(
                        response, content, response_time_ms, expected_status, expected_content
                    )
    ...
    async def _evaluate_response(self, response: aiohttp.ClientResponse, 
                               content: str, response_time_ms: float,
                               expected_status: List[int], 
                               expected_content: Optional[str]) -> HealthCheckResult:
        """응답 평가"""
        # 상태 코드 확인
...
class DatabaseCheck:
    ...
    async def execute(self) -> HealthCheckResult:
        ...
        try:
            if db_type == 'postgresql':
                return await self._check_postgresql(connection_string, test_query, start_time)
            elif db_type == 'mysql':
                return await self._check_mysql(connection_string, test_query, start_time)
            elif db_type == 'sqlite':
                return await self._check_sqlite(connection_string, test_query, start_time)
            elif db_type == 'redis':
    ...
    async def _check_postgresql(self, connection_string: str, test_query: str, start_time: float) -> HealthCheckResult:
        """PostgreSQL 체크"""
        try:
            import asyncpg
    ...
    async def _check_mysql(self, connection_string: str, test_query: str, start_time: float) -> HealthCheckResult:
        """MySQL 체크"""
        try:
            import aiomysql
    ...
    async def _check_sqlite(self, connection_string: str, test_query: str, start_time: float) -> HealthCheckResult:
        """SQLite 체크"""
        try:
            import aiosqlite
    ...
    async def _check_redis(self, connection_string: str, start_time: float) -> HealthCheckResult:
        """Redis 체크"""
        try:
            import aioredis
...
class ServiceCheck:
    ...
    async def _check_udp_port(self, start_time: float) -> HealthCheckResult:
        """UDP 포트 체크"""
        # UDP는 연결이 없는 프로토콜이므로 실제 응답 확인이 어려움
        # 여기서는 소켓 생성만 테스트
        host = self.config.config['host']
        port = self.config.config['port']
...
class CustomCheck:
    ...
    async def execute(self) -> HealthCheckResult:
        ...
        try:
            ...
            # 결과가 HealthCheckResult인 경우 그대로 반환
...
class HealthChecker:
    ...
    def __init__(self, config: Optional[HealthCheckConfig] = None):
        ...
        # 체크 결과 저장
        self.check_results: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=self.config.max_check_history)
        )
        ...
        self.check_states: Dict[str, HealthStatus] = defaultdict(lambda: HealthStatus.UNKNOWN)
    ...
    def add_check(self, check: HealthCheck) -> Result[bool, str]:
        ...
        try:
            ...
            if check.check_type == CheckType.HTTP:
                self.check_handlers[check.name] = EndpointCheck(check)
            elif check.check_type == CheckType.DATABASE:
                self.check_handlers[check.name] = DatabaseCheck(check)
            elif check.check_type == CheckType.SERVICE:
                self.check_handlers[check.name] = ServiceCheck(check)
            elif check.check_type == CheckType.RESOURCE:
                self.check_handlers[check.name] = ResourceCheck(check)
            elif check.check_type == CheckType.CUSTOM:
                if check.name in self.custom_functions:
                    self.check_handlers[check.name] = CustomCheck(check, self.custom_functions[check.name])
                else:
                    return Failure(f"Custom check function not registered for: {check.name}")
            else:
    ...
    def register_custom_check(self, name: str, check_function: Callable) -> None:
        """커스텀 체크 함수 등록"""
        self.custom_functions[name] = check_function
    ...
    def _update_check_state(self, check_name: str, result: HealthCheckResult) -> None:
        ...
        if self.consecutive_failures[check_name] >= self.config.unhealthy_threshold:
            self.check_states[check_name] = HealthStatus.UNHEALTHY
        elif self.consecutive_successes[check_name] >= self.config.healthy_threshold:
            self.check_states[check_name] = HealthStatus.HEALTHY
        else:
            # 현재 결과 상태 유지
            self.check_states[check_name] = result.status
        ...
        for check_name, task in tasks:
            try:
                ...
                if result.is_success():
                    results[check_name] = result.unwrap()
                else:
                    # 실패한 체크에 대한 기본 결과 생성
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        for check_name, result in self.latest_results.items():
            ...
            if result.status == HealthStatus.UNHEALTHY:
                ...
                healthy_services.append(check_name)
        ...
        return {
            ...
            'statistics': {
                ...
                'success_rate': self.total_checks_passed / max(1, self.total_checks_run)
            }
        }
...
async def run_health_checks(checks: List[HealthCheck], 
                          config: Optional[HealthCheckConfig] = None) -> Result[Dict[str, Any], str]:
    """헬스 체크 실행"""
