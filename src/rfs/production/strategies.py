...
class DeploymentStrategy(ABC):
    ...
    def should_rollback(self, metrics: Dict[str, float]) -> bool:
        ...
        if metrics.get("error_rate", 0) > self.config.rollback_on_error_rate:
            self.metrics.rollback_reason = f"High error rate: {metrics['error_rate']:.2%}"
            return True
            
        if metrics.get("latency_p99", 0) > self.config.rollback_on_latency:
            self.metrics.rollback_reason = f"High latency: {metrics['latency_p99']:.2f}s"
            return True
...
class BlueGreenStrategy(DeploymentStrategy):
    ...
    async def deploy(
        ...
        **kwargs
        ...
        try:
            logger.info(f"Starting Blue-Green deployment for {service_name} v{new_version}")
            ...
            health_result = await self.health_check(f"{service_name}-green", new_version)
            
            if isinstance(health_result, Failure):
                return Failure(f"Green environment health check failed: {health_result.error}")
            ...
            # 4. 메트릭 확인
            metrics_result = await self.collect_metrics(f"{service_name}-green", new_version)
            ...
            if self.should_rollback(metrics_result.value):
                ...
                return Failure(f"Deployment rolled back: {self.metrics.rollback_reason}")
            ...
            if isinstance(final_health, Failure):
                await self.rollback(service_name)
            ...
            self.metrics.deployment_duration = self.metrics.end_time - self.metrics.start_time
            ...
            logger.info(f"Blue-Green deployment completed successfully in {self.metrics.deployment_duration}")
        ...
        **kwargs
        ...
        try:
            ...
            while current_percentage <= 100:
                ...
                if isinstance(health_result, Failure):
                    await self.rollback(service_name)
                    return Failure(f"Health check failed at {current_percentage}%: {health_result.error}")
                ...
                current_percentage = min(100, current_percentage + self.config.canary_increment)
            ...
            logger.info(f"Canary deployment completed successfully in {self.metrics.deployment_duration}")
            return Success(self.metrics)
    ...
    async def deploy(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Starting Rolling deployment for {service_name} v{new_version} ({total_instances} instances)")
            
            deployed_instances = 0
            
            while deployed_instances < total_instances:
                # 배치 크기 계산
                batch_size = min(self.config.batch_size, total_instances - deployed_instances)
                
                # 1. 배치 배포
                logger.info(f"Deploying batch {deployed_instances + 1}-{deployed_instances + batch_size}/{total_instances}...")
                await asyncio.sleep(2)  # 시뮬레이션
                ...
                for i in range(batch_size):
                    ...
                    if isinstance(health_result, Failure):
                        await self.rollback(service_name)
                        return Failure(f"Instance {instance_name} health check failed: {health_result.error}")
                ...
                if self.should_rollback(metrics_result.value):
                    logger.warning(f"Rollback triggered at {deployed_instances}/{total_instances}")
                    await self.rollback(service_name)
                    self.metrics.rollback_triggered = True
                    return Failure(f"Deployment rolled back: {self.metrics.rollback_reason}")
                ...
                if deployed_instances < total_instances:
                    logger.info(f"Waiting {self.config.batch_interval}s before next batch...")
            ...
            logger.info(f"Rolling deployment completed successfully in {self.metrics.deployment_duration}")
            return Success(self.metrics)
