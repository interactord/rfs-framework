...
class MessageBroker(ABC):
    ...
    # 배치 처리
    async def publish_batch(self, topic: str, messages: List[Message]) -> Result[None, str]:
        """배치 메시지 발행"""
        try:
    ...
    async def nack_message(self, message: Message, requeue: bool = True) -> Result[None, str]:
        """메시지 거부"""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        error_rate = (self._stats["errors"] / total_messages) if total_messages > 0 else 0
    ...
    async def remove_broker(self, name: str) -> Result[None, str]:
        ...
        try:
            ...
            if not disconnect_result.is_success():
                logger.warning(f"브로커 연결 해제 실패: {disconnect_result.unwrap_err()}")
...
# 브로커 팩토리
async def create_message_broker(config: MessageConfig, name: str = "default") -> Result[MessageBroker, str]:
    """메시지 브로커 생성"""
    try:
        if config.broker_type == BrokerType.REDIS:
            from .redis_broker import RedisMessageBroker
            broker = RedisMessageBroker(config)
        elif config.broker_type == BrokerType.MEMORY:
        ...
        elif config.broker_type == BrokerType.GOOGLE_PUBSUB:
