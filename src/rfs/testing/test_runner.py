    ...
    def generate_summary(self) -> str:
        ...
        if self.get_failed_tests():
            ...
            for result in self.get_failed_tests():
                summary.append(f"  - {result.test_name}: {result.message}")
...
class TestSuite:
    ...
    def add_test(self, test_case: Union[TestCase, Callable]):
        ...
        if isinstance(test_case, TestCase):
            ...
            self.test_cases.append(wrapped_case)
        else:
            raise ValueError("테스트는 TestCase 인스턴스 또는 호출 가능 객체여야 합니다")
    ...
    async def run_test_case(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            if test_case.setup_done:
                try:
                    ...
                    await logger.log_warning(f"테스트 정리 실패: {test_case.name} - {str(e)}")
        ...
        if self.verbose:
            if test_result.is_passed():
                ...
                await logger.log_error(f"❌ {test_case.name} ({duration:.3f}s): {message}")
            ...
            if spec and spec.loader:
                ...
                for name in dir(module):
                    ...
                    if inspect.isclass(obj) and issubclass(obj, TestCase) and obj != TestCase:
                        ...
                        # 테스트 함수
                        test_cases.append(FunctionTestCase(obj, name))
...
async def run_test(test_case: Union[TestCase, Callable], verbose: bool = True) -> TestResult:
    """단일 테스트 실행"""
...
async def run_test_suite(test_suite: TestSuite, verbose: bool = True, 
                        stop_on_failure: bool = False) -> TestReport:
    """테스트 스위트 실행"""
...
async def coverage_report(test_suite: TestSuite) -> Dict[str, Any]:
    ...
    return {
        ...
        "coverage": {
            "line_coverage": 0.0,  # 실제 구현 필요
            "branch_coverage": 0.0,  # 실제 구현 필요
            "function_coverage": 0.0  # 실제 구현 필요
        },
