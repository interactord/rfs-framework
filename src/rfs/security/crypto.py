...
class CryptoManager:
    ...
    def _check_cryptography(self) -> Result[None, str]:
        ...
        if not CRYPTOGRAPHY_AVAILABLE:
            return Failure("cryptography 라이브러리가 필요합니다: pip install cryptography")
        return Success(None)
    ...
    def encrypt_aes_gcm(self, data: bytes, key: bytes, nonce: Optional[bytes] = None) -> Result[EncryptionResult, str]:
        """AES-GCM 암호화"""
    ...
    def decrypt_aes_gcm(self, encrypted_result: EncryptionResult, key: bytes) -> Result[bytes, str]:
        """AES-GCM 복호화"""
        check_result = self._check_cryptography()
        if check_result.is_failure():
            return check_result
    ...
    def encrypt_aes_cbc(self, data: bytes, key: bytes, iv: Optional[bytes] = None) -> Result[EncryptionResult, str]:
        """AES-CBC 암호화"""
        check_result = self._check_cryptography()
        if check_result.is_failure():
            return check_result
    ...
    def decrypt_aes_cbc(self, encrypted_result: EncryptionResult, key: bytes) -> Result[bytes, str]:
        """AES-CBC 복호화"""
        check_result = self._check_cryptography()
        if check_result.is_failure():
            return check_result
    ...
    def decrypt_rsa(self, encrypted_data: bytes, private_key_pem: bytes) -> Result[bytes, str]:
        """RSA 복호화"""
        check_result = self._check_cryptography()
        if check_result.is_failure():
            return check_result
    ...
    def verify_signature(self, data: bytes, signature: bytes, public_key_pem: bytes) -> Result[bool, str]:
        """서명 검증"""
        check_result = self._check_cryptography()
        if check_result.is_failure():
            return check_result
    ...
    def hash_data(self, data: bytes, algorithm: HashAlgorithm = HashAlgorithm.SHA256, 
                  salt: Optional[bytes] = None) -> Result[bytes, str]:
        """데이터 해싱"""
        try:
            if algorithm == HashAlgorithm.SHA256:
                ...
                if salt:
                    hasher = hashlib.blake2b(salt=salt[:16])  # blake2b는 최대 16바이트 솔트
                else:
    ...
    def verify_hash(self, data: bytes, hashed_data: bytes, 
                   algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> Result[bool, str]:
        """해시 검증"""
        try:
...
def encrypt(data: Union[str, bytes], key: bytes, 
           algorithm: EncryptionAlgorithm = EncryptionAlgorithm.AES_256_GCM) -> Result[EncryptionResult, str]:
    """데이터 암호화"""
...
def decrypt(encrypted_result: EncryptionResult, key: bytes, 
           algorithm: EncryptionAlgorithm = EncryptionAlgorithm.AES_256_GCM) -> Result[bytes, str]:
    """데이터 복호화"""
    if algorithm == EncryptionAlgorithm.AES_256_GCM:
...
def hash_password(password: str, salt: Optional[bytes] = None) -> Result[str, str]:
    """비밀번호 해싱"""
    hash_result = _crypto_manager.hash_data(password.encode(), HashAlgorithm.PBKDF2, salt)
...
def verify_password(password: str, hashed_password: str) -> Result[bool, str]:
    ...
    try:
        ...
        return _crypto_manager.verify_hash(password.encode(), hashed_bytes, HashAlgorithm.PBKDF2)
    except Exception as e:
...
def hash_data(data: Union[str, bytes], algorithm: HashAlgorithm = HashAlgorithm.SHA256, 
             salt: Optional[bytes] = None) -> Result[str, str]:
    """데이터 해싱 (Base64 인코딩)"""
    if isinstance(data, str):
        data = data.encode('utf-8')
...
def verify_hash(data: Union[str, bytes], hashed_data: str, 
               algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> Result[bool, str]:
    """해시 검증"""
    if isinstance(data, str):
        data = data.encode('utf-8')
...
def verify_signature(data: Union[str, bytes], signature: str, public_key: bytes) -> Result[bool, str]:
    """서명 검증"""
    if isinstance(data, str):
        data = data.encode('utf-8')
