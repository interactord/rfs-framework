...
class IOBottleneck:
    ...
    type: str  # "disk", "network", "filesystem"
    description: str
...
class IOMetrics:
    ...
    detected_bottlenecks: List[IOBottleneck] = field(default_factory=list)
    ...
    def calculate_io_rates(self, minutes: int = 1) -> Dict[str, float]:
        ...
        return {
            "disk_read_rate_mbps": (last.total_disk_reads - first.total_disk_reads) / (1024**2) / time_diff,
            "disk_write_rate_mbps": (last.total_disk_writes - first.total_disk_writes) / (1024**2) / time_diff,
            "network_send_rate_mbps": (last.total_network_sent - first.total_network_sent) / (1024**2) / time_diff,
            "network_recv_rate_mbps": (last.total_network_recv - first.total_network_recv) / (1024**2) / time_diff
        }
...
class IOProfiler:
            ...
            try:
                ...
                if disk_io:
                    for device, stats in disk_io.items():
                        ...
                        disk_io_stats.append(disk_stat)
                        total_disk_reads += stats.read_bytes
                        total_disk_writes += stats.write_bytes
            except Exception as e:
            ...
            try:
                ...
                if network_io:
                    for interface, stats in network_io.items():
                        ...
                        network_io_stats.append(network_stat)
                        total_network_sent += stats.bytes_sent
                        total_network_recv += stats.bytes_recv
            except Exception as e:
            ...
            try:
                ...
                for partition in partitions:
                    try:
                        ...
                        filesystem_usage.append(fs_usage)
            ...
            try:
                ...
                for proc in psutil.process_iter(['pid', 'name']):
                    try:
                        ...
                        if io_info and (io_info.read_bytes > 0 or io_info.write_bytes > 0):
                            processes.append({
                                'pid': proc.info['pid'],
                                'name': proc.info['name'],
                                'io_info': io_info
                ...
                for proc_info in processes[:self.top_processes_count]:
                    ...
                    top_io_processes.append(ProcessIOInfo(
                        pid=proc_info['pid'],
    ...
    async def _detect_io_bottlenecks(self, snapshot: IOSnapshot):
        ...
        try:
            ...
            for bottleneck in bottlenecks:
                self.metrics.detected_bottlenecks.append(bottleneck)
    ...
    async def _check_io_alerts(self, snapshot: IOSnapshot):
        ...
        try:
            ...
            for fs in snapshot.filesystem_usage:
                if fs.percent > 90.0:
                    alerts.append(f"High disk usage on {fs.mountpoint}: {fs.percent:.1f}%")
                ...
                if network.errin + network.errout > 0:
                    ...
                    if total_packets > 0:
                        ...
                        if error_rate > 0.01:  # 1%
                            alerts.append(f"Network errors on {network.interface}: {error_rate:.2%}")
            
            # I/O 속도 급증 알림 (이전 측정치와 비교)
            if len(self.metrics.snapshots) > 1:
                prev = self.metrics.snapshots[-2]
                time_diff = (snapshot.timestamp - prev.timestamp).total_seconds()
                
                if time_diff > 0:
                    disk_read_rate = (snapshot.total_disk_reads - prev.total_disk_reads) / time_diff / (1024**2)
                    disk_write_rate = (snapshot.total_disk_writes - prev.total_disk_writes) / time_diff / (1024**2)
                    
                    if disk_read_rate > 100:  # 100MB/s
                        alerts.append(f"High disk read rate: {disk_read_rate:.1f} MB/s")
                    if disk_write_rate > 100:  # 100MB/s
                        alerts.append(f"High disk write rate: {disk_write_rate:.1f} MB/s")
    ...
    def get_detected_bottlenecks(self, minutes: int = 60) -> List[IOBottleneck]:
        ...
        cutoff_time = datetime.now() - timedelta(minutes=minutes)
        return [
            bottleneck for bottleneck in self.metrics.detected_bottlenecks
        ...
        try:
            ...
            if recent[-1].filesystem_usage:
                for fs in recent[-1].filesystem_usage:
                    analysis["filesystem_analysis"][fs.mountpoint] = {
                        "usage_percent": fs.percent,
                        "free_gb": fs.free / (1024**3),
                        "fstype": fs.fstype
                    }
            ...
            if recent[-1].network_io:
                for network in recent[-1].network_io:
                    ...
                    analysis["network_analysis"][network.interface] = {
                        "mb_sent": network.bytes_sent / (1024**2),
                        "mb_recv": network.bytes_recv / (1024**2),
                        "error_rate": error_rate,
                        "packet_loss_rate": (network.dropin + network.dropout) / total_packets if total_packets > 0 else 0.0
                    }
...
# 편의 함수들
def create_io_profiler(collection_interval: float = 2.0, top_processes_count: int = 10) -> IOProfiler:
    """I/O 프로파일러 생성"""
    return IOProfiler(collection_interval=collection_interval, top_processes_count=top_processes_count)
