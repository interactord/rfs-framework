...
class AnnotationProcessor:
        ...
        for annotation in annotations:
            match annotation.annotation_type:
                ...
                case "Controller":
                    ...
                    metadata.metadata = {**metadata.metadata, 'route': annotation.get_param('route')}
                    metadata.metadata = {**metadata.metadata, 'methods': annotation.get_param('method')}
                case "Service":
    ...
    def _analyze_constructor(self, cls: Type, metadata: ComponentMetadata):
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param.annotation != inspect.Parameter.empty:
                dep = DependencyMetadata(name=param_name, type=param.annotation, required=param.default == inspect.Parameter.empty, injection_type=InjectionType.CONSTRUCTOR)
            ...
            for field_name, field_type in cls.__annotations__.items():
                if hasattr(cls, field_name):
                    ...
                    if hasattr(field_value, '_autowired'):
                        dep = DependencyMetadata(name=field_name, type=field_type, injection_type=InjectionType.FIELD, qualifier=getattr(field_value, '_qualifier', None), lazy=getattr(field_value, '_lazy', False))
                        metadata.add_dependency(dep)
                    elif hasattr(field_value, '_value_key'):
                        config_values = metadata.metadata.setdefault('config_values', {})
                        config_values[field_name] = {'key': field_value._value_key, 'default': field_value._value_default}
        ...
        for name, method in inspect.getmembers(cls, inspect.ismethod):
            if hasattr(method, '_autowired_setter'):
                ...
                if len(params) == 2:
                    param = params[1]
                    if param.annotation != inspect.Parameter.empty:
                        dep = DependencyMetadata(name=name, type=param.annotation, injection_type=InjectionType.SETTER)
                        metadata.add_dependency(dep)
    ...
    def validate_dependencies(self, cls: Type) -> List[str]:
        ...
        if self._check_circular_dependency(metadata.component_id, set()):
            errors = errors + [f'Circular dependency detected for {metadata.component_id}']
        for dep in metadata.dependencies:
            if dep.required:
                dep_result = self.registry.get_component(dep.name)
                if not dep_result or type(dep_result).__name__ == 'Failure':
                    errors = errors + [f'Required dependency {dep.name} not found for {metadata.component_id}']
        return errors
    ...
    def _check_circular_dependency(self, component_id: str, visited: Set[str]) -> bool:
        ...
        for cls in [*self.registry.components.values(), *self.registry.services.values(), *self.registry.repositories.values()]:
            metadata = get_component_metadata(cls)
...
def validate_configuration() -> List[str]:
    ...
    all_components = []
    for storage in [registry.components, registry.services, registry.repositories, registry.use_cases, registry.controllers]:
