...
import inspect
import logging
from typing import Any, Callable, Optional, List, Union, Dict, Type
from dataclasses import dataclass, field

from .transactions import (
    TransactionConfig, RedisTransactionConfig, DistributedTransactionConfig,
    TransactionContext, TransactionType, IsolationLevel,
    get_transaction_registry, execute_with_retry
)
...
def Transactional(isolation: IsolationLevel = IsolationLevel.READ_COMMITTED,
                 timeout: int = 30,
                 retry_count: int = 3,
                 ...
                 readonly: bool = False,
                 propagation: str = "REQUIRED") -> Callable:
    """
    데이터베이스 트랜잭션 데코레이터
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs) -> Any:
            return await _execute_transactional(func, config, TransactionType.DATABASE, *args, **kwargs)
        
        @functools.wraps(func)
        def sync_wrapper(*args, **kwargs) -> Any:
            return asyncio.run(_execute_transactional(func, config, TransactionType.DATABASE, *args, **kwargs))
...
def RedisTransaction(ttl: Optional[int] = None,
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        async def async_wrapper(*args, **kwargs) -> Any:
            return await _execute_transactional(func, config, TransactionType.REDIS, *args, **kwargs)
        
        @functools.wraps(func)
        def sync_wrapper(*args, **kwargs) -> Any:
            return asyncio.run(_execute_transactional(func, config, TransactionType.REDIS, *args, **kwargs))
...
def DistributedTransaction(saga_id: str,
                         timeout: int = 300,
                         retry_count: int = 3,
                         compensation_timeout: int = 60,
                         idempotent: bool = True) -> Callable:
    """
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        async def async_wrapper(*args, **kwargs) -> Any:
            return await _execute_transactional(func, config, TransactionType.DISTRIBUTED, *args, **kwargs)
        
        @functools.wraps(func)
        def sync_wrapper(*args, **kwargs) -> Any:
            return asyncio.run(_execute_transactional(func, config, TransactionType.DISTRIBUTED, *args, **kwargs))
...
async def _execute_transactional(func: Callable, 
                                config: Union[TransactionConfig, RedisTransactionConfig, DistributedTransactionConfig],
...
class TransactionalMethod:
    ...
    def __call__(self, cls: type) -> type:
        ...
        for attr_name in dir(cls):
            ...
            if (inspect.isfunction(attr) or inspect.ismethod(attr)) and not attr_name.startswith('_'):
...
def transactional_context(transaction_type: TransactionType, 
                         config: Union[TransactionConfig, RedisTransactionConfig, DistributedTransactionConfig]):
    """
...
class TransactionalContextManager:
    ...
    async def __aenter__(self):
        ...
        if not self.manager:
            raise RuntimeError(f"No transaction manager for {self.transaction_type.value}")
...
class TransactionStatus:
    ...
    def register_transaction(cls, context: TransactionContext):
        """트랜잭션 등록"""
        cls._active_transactions[context.transaction_id] = context
    
    @classmethod
    def unregister_transaction(cls, transaction_id: str):
        """트랜잭션 등록 해제"""
        cls._active_transactions.pop(transaction_id, None)
    ...
    async def main():
        ...
        class MockManager(DatabaseTransactionManager):
            async def execute_in_transaction(self, context, func, *args, **kwargs):
                print(f"Executing {func.__name__} in transaction {context.transaction_id}")
                if asyncio.iscoroutinefunction(func):
