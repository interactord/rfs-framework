...
class CPUProfiler:
    ...
    def __init__(self, collection_interval: float = 1.0, top_processes_count: int = 10):
        ...
        # CPU 바운드 탐지 설정
        self.cpu_bound_threshold = 80.0  # CPU 사용률 임계값
        self.cpu_bound_duration = 5.0    # 지속 시간 (초)
    ...
    async def _collect_cpu_snapshot(self) -> Optional[CPUSnapshot]:
        ...
        try:
            ...
            for i, usage in enumerate(core_percentages):
                ...
                core_usage.append(CPUCoreUsage(
                    core_id=i,
                    usage_percent=usage,
                    frequency=freq
                ))
            ...
            try:
                processes = []
                for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'cpu_times', 'num_threads']):
                    try:
                        info = proc.info
                ...
                for proc_info in processes[:self.top_processes_count]:
                    cpu_times_dict = {}
                    if proc_info['cpu_times']:
                        cpu_times_dict = proc_info['cpu_times']._asdict()
                    
                    top_processes.append(ProcessCPUInfo(
                        pid=proc_info['pid'],
    ...
    async def _detect_cpu_bound_tasks(self, snapshot: CPUSnapshot):
        ...
        try:
            if snapshot.overall_cpu_percent > self.cpu_bound_threshold:
                # 최근 스냅샷들에서 지속적인 높은 CPU 사용률 확인
                recent_snapshots = self.metrics.get_recent_snapshots(1)  # 최근 1분
                
                if len(recent_snapshots) >= self.cpu_bound_duration / self.collection_interval:
                    high_cpu_count = sum(
                        1 for s in recent_snapshots[-int(self.cpu_bound_duration / self.collection_interval):]
                        if s.overall_cpu_percent > self.cpu_bound_threshold
                    )
                    
                    if high_cpu_count >= self.cpu_bound_duration / self.collection_interval * 0.8:  # 80% 이상
                        # CPU 바운드 작업 탐지
                        detection = {
                            "timestamp": datetime.now(),
                            "duration_seconds": self.cpu_bound_duration,
                            "avg_cpu_percent": sum(s.overall_cpu_percent for s in recent_snapshots) / len(recent_snapshots),
                            "top_processes": [proc.to_dict() for proc in snapshot.top_processes[:5]]
                        }
                        
                        self.metrics.cpu_bound_detections.append(detection)
    ...
    async def _check_cpu_alerts(self, snapshot: CPUSnapshot):
        ...
        try:
            ...
            if snapshot.overall_cpu_percent > 85.0:
                alerts.append(f"High CPU usage: {snapshot.overall_cpu_percent:.1f}%")
            
            # 높은 로드 평균 (코어 수 대비)
            if snapshot.loadavg and len(snapshot.loadavg) > 0:
                load_1min = snapshot.loadavg[0]
                if load_1min > self.cpu_count * 1.5:  # 코어 수의 1.5배 초과
                    alerts.append(f"High load average: {load_1min:.2f} (cores: {self.cpu_count})")
            ...
            if len(snapshot.core_usage) > 1:
                ...
                if max_usage - min_usage > 50.0:  # 50% 이상 차이
                    alerts.append(f"Unbalanced core usage: {max_usage:.1f}% - {min_usage:.1f}%")
    ...
    async def stop_profiling(self) -> Result[List[ProfileResult], str]:
        ...
        try:
            ...
            results = []
            for func_info, (total_calls, _, total_time, cumulative_time) in stats.stats.items():
            ...
            self.profiling_results = results[:100]  # 상위 100개 함수만 저장
            self.metrics.profile_results = self.profiling_results
            
            self.current_profiler = None
            
            logger.info(f"CPU function profiling stopped. Analyzed {len(results)} functions")
                ...
                for core_id in range(core_count):
                    ...
                    for snapshot in recent:
                        if core_id < len(snapshot.core_usage):
                            core_values.append(snapshot.core_usage[core_id].usage_percent)
                    
                    if core_values:
                        analysis["core_analysis"][f"core_{core_id}"] = {
...
# 편의 함수들
def create_cpu_profiler(collection_interval: float = 1.0, top_processes_count: int = 10) -> CPUProfiler:
    """CPU 프로파일러 생성"""
    return CPUProfiler(collection_interval=collection_interval, top_processes_count=top_processes_count)
