...
import asyncio
from typing import Dict, Any, List, Optional, Type, Union, Callable, AsyncContextManager
...
# 컨텍스트 변수들
current_session: ContextVar[Optional['DatabaseSession']] = ContextVar('current_session', default=None)
current_transaction: ContextVar[Optional['DatabaseTransaction']] = ContextVar('current_transaction', default=None)
...
class DatabaseSession(ABC):
    ...
    @abstractmethod
    async def execute(self, query: str, params: Dict[str, Any] = None) -> Result[Any, str]:
        """쿼리 실행"""
        pass
...
class SessionManager(metaclass=SingletonMeta):
    ...
    async def create_session(self, database: Database = None) -> Result[DatabaseSession, str]:
        """세션 생성"""
        try:
            if database is None:
            ...
            elif isinstance(database, TortoiseDatabase):
...
# 데코레이터
def with_session(func: Callable = None, database: Database = None, config: SessionConfig = None):
    """세션 데코레이터"""
