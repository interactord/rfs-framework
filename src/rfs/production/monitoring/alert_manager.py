...
class EscalationLevel(Enum):
    ...
    L2 = "level2"    # 2차 에스컬레이션
    L3 = "level3"    # 3차 에스컬레이션
    EXECUTIVE = "executive"  # 경영진 에스컬레이션
...
class AlertCondition:
    ...
    comparison_type: str = "absolute"  # absolute, percentage, rate
...
class AlertPolicy:
    ...
    id: str
    name: str
    rules: List[str]  # AlertRule IDs
    channels: List[str]  # NotificationChannel IDs
    escalation_rules: List[Dict[str, Any]] = field(default_factory=list)
    group_by: List[str] = field(default_factory=list)  # 그룹화 키
...
class AlertChannel:
    ...
    async def send_alert(self, alert: Alert, context: Dict[str, Any] = None) -> Result[bool, str]:
        """알림 전송 (서브클래스에서 구현)"""
...
class EmailChannel(AlertChannel):
    ...
    def _format_labels_section(self, alert: Alert) -> str:
        ...
        for key, value in alert.labels.items():
            labels_html += f"<li><strong>{key}:</strong> {value}</li>"
        labels_html += "</ul></div>"
...
class SlackChannel(AlertChannel):
    ...
    def _format_slack_message(self, alert: Alert, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """슬랙 메시지 포맷"""
        severity_emoji = {
...
class WebhookChannel(AlertChannel):
    ...
    async def send_alert(self, alert: Alert, context: Dict[str, Any] = None) -> Result[bool, str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                if method == 'GET':
                    async with session.get(webhook_url, headers=headers, params=payload) as response:
    ...
    def _format_webhook_payload(self, alert: Alert, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """웹훅 페이로드 포맷"""
        payload = {
            "alert": {
                "id": alert.id,
        ...
        if alert.acknowledged_at:
            payload["alert"]["acknowledged_at"] = alert.acknowledged_at.isoformat()
            payload["alert"]["acknowledged_by"] = alert.acknowledged_by
...
class AlertGrouping:
    ...
    def add_alert(self, alert: Alert, group_by: List[str]) -> str:
        ...
        if group_key not in self.group_timers:
            self.group_timers[group_key] = datetime.now()
    ...
    def _generate_group_key(self, alert: Alert, group_by: List[str]) -> str:
        ...
        for field in group_by:
            if field in alert.labels:
                key_parts.append(f"{field}={alert.labels[field]}")
            elif hasattr(alert, field):
                key_parts.append(f"{field}={getattr(alert, field)}")
    ...
    def get_ready_groups(self, wait_minutes: float) -> Dict[str, List[Alert]]:
        ...
        for group_key, timer in list(self.group_timers.items()):
            if (current_time - timer).total_seconds() >= wait_minutes * 60:
                ready_groups[group_key] = self.groups[group_key].copy()
                # 그룹 정리
...
class EscalationManager:
    ...
    def start_escalation(self, alert: Alert, escalation_rules: List[Dict[str, Any]]) -> None:
        """에스컬레이션 시작"""
        self.escalation_timers[alert.id] = datetime.now()
        self.escalated_alerts[alert.id] = EscalationLevel.L1
    
    def check_escalations(self, escalation_rules: List[Dict[str, Any]]) -> List[Tuple[str, EscalationLevel]]:
        """에스컬레이션 확인"""
        escalations = []
        current_time = datetime.now()
        ...
        for alert_id, start_time in list(self.escalation_timers.items()):
            ...
            for rule in escalation_rules:
                ...
                # 현재 레벨보다 높고, 대기 시간이 지났으면 에스컬레이션
                if (rule_level.value > current_level.value and
                    (current_time - start_time).total_seconds() >= wait_minutes * 60):
                    
                    self.escalated_alerts[alert_id] = rule_level
                    escalations.append((alert_id, rule_level))
                    break
    ...
    def stop_escalation(self, alert_id: str) -> None:
        """에스컬레이션 중지"""
        self.escalation_timers.pop(alert_id, None)
        self.escalated_alerts.pop(alert_id, None)
...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.channel_stats: Dict[str, Dict[str, int]] = defaultdict(lambda: {"sent": 0, "failed": 0})
    ...
    def add_channel(self, channel: NotificationChannel) -> Result[bool, str]:
        ...
        try:
            if channel.channel_type not in self.channel_handlers:
                return Failure(f"Unsupported channel type: {channel.channel_type.value}")
    ...
    async def create_alert(self, rule_id: str, title: str, message: str,
                          source: str = "system", labels: Dict[str, str] = None,
                          annotations: Dict[str, str] = None) -> Result[Alert, str]:
        """알림 생성"""
        try:
            if rule_id not in self.rules:
                return Failure(f"Rule not found: {rule_id}")
            ...
            if existing_alert:
                return Success(existing_alert)  # 기존 알림 반환
    ...
    async def _process_alert(self, alert: Alert) -> None:
        """알림 처리"""
        # 해당 알림 규칙을 사용하는 정책 찾기
        applicable_policies = [
            policy for policy in self.policies.values()
            if alert.rule_id in policy.rules
        ]
    ...
    async def acknowledge_alert(self, alert_id: str, acknowledged_by: str) -> Result[bool, str]:
        """알림 확인"""
        try:
    ...
    async def resolve_alert(self, alert_id: str, resolved_by: str = "system") -> Result[bool, str]:
        """알림 해결"""
        try:
            if alert_id not in self.active_alerts:
                return Failure(f"Alert not found: {alert_id}")
    ...
    async def _process_grouped_alerts(self) -> None:
        ...
        for policy in self.policies.values():
            ready_groups = self.alert_grouping.get_ready_groups(policy.group_wait_minutes)
            
            for group_key, alerts in ready_groups.items():
                await self._send_grouped_alerts(policy, alerts)
    
    async def _send_grouped_alerts(self, policy: AlertPolicy, alerts: List[Alert]) -> None:
        """그룹화된 알림 전송"""
        try:
            for channel_id in policy.channels:
                if channel_id not in self.channels:
    ...
    async def _send_to_channel(self, channel_config: NotificationChannel, 
                              alerts: List[Alert]) -> None:
        """채널로 알림 전송"""
        try:
            channel_class = self.channel_handlers.get(channel_config.channel_type)
            if not channel_class:
                logging.error(f"Unsupported channel type: {channel_config.channel_type}")
                return
            ...
            for alert in alerts:
                try:
                    ...
                    if result.is_success():
                        ...
                        logging.info(f"Alert sent to {channel_config.name}: {alert.title}")
                    else:
                        self.channel_stats[channel_config.id]["failed"] += 1
                        logging.error(f"Failed to send alert to {channel_config.name}: {result.error}")
    ...
    async def _process_escalations(self) -> None:
        ...
        for policy in self.policies.values():
            ...
            escalations = self.escalation_manager.check_escalations(policy.escalation_rules)
            ...
            for alert_id, new_level in escalations:
                if alert_id in self.active_alerts:
                    ...
                    # 에스컬레이션 레벨에 따른 추가 알림 전송
                    await self._send_escalation_alert(alert, policy)
    
    async def _send_escalation_alert(self, alert: Alert, policy: AlertPolicy) -> None:
        """에스컬레이션 알림 전송"""
        # 에스컬레이션 레벨에 따른 추가 채널이나 메시지 수정
        escalation_message = f"[ESCALATED - {alert.escalation_level.value.upper()}] {alert.message}"
...
async def setup_alerting(rules: List[AlertRule], channels: List[NotificationChannel], 
                        policies: List[AlertPolicy]) -> Result[AlertManager, str]:
    """알림 시스템 설정"""
