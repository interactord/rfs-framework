...
class VersionCommand(Command):
    ...
    async def execute(self, ctx: CommandContext) -> Result[str, str]:
        ...
        try:
            ...
            if ctx.console:
                ctx.console.print(f"[bold green]RFS Framework v{info['version']}[/bold green]")
                ctx.console.print(f"Phase: {info['phase']}")
                ctx.console.print(f"Python: {sys.version_info.major}.{sys.version_info.minor}")
                ctx.console.print(f"Modules: {info['total_modules']}")
                ctx.console.print(f"Production Ready: {'✅' if info['production_ready'] else '❌'}")
                ctx.console.print(f"Cloud Run Ready: {'✅' if info['cloud_run_ready'] else '❌'}")
            else:
                print(f"RFS Framework v{info['version']}")
                print(f"Phase: {info['phase']}")
                print(f"Python: {sys.version_info.major}.{sys.version_info.minor}")
            
            return Success(f"RFS Framework v{info['version']}")
...
class StatusCommand(Command):
    ...
    async def execute(self, ctx: CommandContext) -> Result[str, str]:
        ...
        try:
            status = {
                ...
                "project_root": str(ctx.project_root) if ctx.project_root else "Not detected",
            ...
            if ctx.console:
                ...
                ctx.console.print(f"Framework: [green]{status['framework']}[/green]")
                ctx.console.print(f"Python: [blue]{status['python_version']}[/blue]")
                ctx.console.print(f"Project: [yellow]{status['project_root']}[/yellow]")
                ctx.console.print(f"Environment: [cyan]{status['environment']}[/cyan]")
    ...
    def _check_dependencies(self) -> Dict[str, bool]:
        ...
        try:
            ...
            deps['google-cloud-run'] = True
        except ImportError:
            deps['google-cloud-run'] = False
...
class ConfigCommand(Command):
    ...
    async def execute(self, ctx: CommandContext) -> Result[str, str]:
        """설정 명령어 실행"""
        try:
            action = ctx.args.get('positional', ['show'])[0] if ctx.args.get('positional') else 'show'
    ...
    async def _show_config(self, ctx: CommandContext) -> Result[str, str]:
        ...
        try:
            if ctx.console:
                ...
                ctx.console.print(f"Project Root: [yellow]{ctx.project_root or 'None'}[/yellow]")
    ...
    async def execute(self, ctx: CommandContext) -> Result[str, str]:
        ...
        try:
            if ctx.console:
                ...
                ctx.console.print("Enterprise-Grade Reactive Functional Serverless Framework")
                ...
                ctx.console.print("  [yellow]--verbose, -v[/yellow]     Enable verbose output")
                ctx.console.print("  [yellow]--dry-run[/yellow]         Show what would be done")
                ctx.console.print("  [yellow]--env ENV[/yellow]         Set environment")
            else:
