...
class HandlerMode(Enum):
    ...
    SYNC = "sync"           # 동기 처리
    ASYNC = "async"         # 비동기 처리
    FIRE_AND_FORGET = "fire_and_forget"  # 비동기 처리 (결과 무시)
...
class FunctionEventHandler(EventHandler):
    ...
    def __init__(
        ...
        self._handler_id = handler_id or f"{handler_func.__module__}.{handler_func.__name__}"
...
class HandlerRegistry:
    ...
    def register_handler(
        ...
        try:
            ...
            self._handler_metadata[handler_id] = metadata
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for metadata in self._handler_metadata.values():
            handlers_by_priority[metadata.priority.value] += 1
            handlers_by_mode[metadata.mode.value] += 1
...
class EventProcessor:
    ...
    def __init__(self, registry: HandlerRegistry):
        ...
        self._execution_history: deque = deque(maxlen=1000)  # 최근 1000개 실행 기록
    ...
    async def process_event(
        ...
        if filter_tags:
            handlers = [
                h for h in handlers 
                if self._handler_matches_tags(h, filter_tags)
            ]
    ...
    async def process_event_parallel(
        ...
        for result in results:
            if isinstance(result, Exception):
                processed_results.append(Failure(f"Handler execution failed: {result}"))
            else:
                processed_results.append(result)
        
        return processed_results
    
    def _handler_matches_tags(self, handler: EventHandler, filter_tags: Set[str]) -> bool:
        """핸들러가 태그와 일치하는지 확인"""
        handler_id = handler.get_handler_id()
    ...
    async def _execute_handler(
        ...
        try:
            ...
            # 모든 재시도 실패
            failure_result = Failure(f"Handler failed after {max_retries} attempts: {last_error}")
    ...
    def _update_handler_stats(
        ...
        if execution.execution_time_ms:
            metadata.total_execution_time_ms += execution.execution_time_ms
...
def event_handler(
    ...
    def decorator(func: Callable[[Event], Union[Any, Result[Any, str]]]):
        ...
        # 전역 레지스트리에 등록 (지연 등록을 위해 함수에 메타데이터 저장)
