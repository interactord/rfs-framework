...
class TransactionSynchronization:
    ...
    def register_before_commit(self, action: Callable):
        """커밋 전 액션 등록"""
        self.before_commit_actions.append(action)
    
    def register_after_commit(self, action: Callable):
        """커밋 후 액션 등록"""
        self.after_commit_actions.append(action)
    
    def register_after_rollback(self, action: Callable):
        """롤백 후 액션 등록"""
        self.after_rollback_actions.append(action)
    
    def register_after_completion(self, action: Callable):
        """완료 후 액션 등록"""
        self.after_completion_actions.append(action)
    ...
    def clear(self):
        """모든 액션 클리어"""
        self.before_commit_actions.clear()
        self.after_commit_actions.clear()
        self.after_rollback_actions.clear()
        self.after_completion_actions.clear()