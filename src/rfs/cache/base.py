    ...
    async def remove_cache(self, name: str) -> Result[None, str]:
        ...
        try:
            ...
            if self.default_cache == name:
                self.default_cache = next(iter(self.caches.keys())) if self.caches else None
...
# 캐시 팩토리
async def create_cache(config: CacheConfig, name: str = "default") -> Result[CacheBackend, str]:
    """캐시 생성"""
    try:
