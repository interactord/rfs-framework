...
class BackupType(Enum):
    ...
    SNAPSHOT = "snapshot"  # 스냅샷 백업
    CONTINUOUS = "continuous"  # 지속적 백업
...
class BackupManager:
    ...
    async def restore_backup(
        ...
        try:
            ...
            self.restore_operations[operation.id] = operation
        ...
        try:
            if self.storage_config.type == StorageType.LOCAL:
                ...
                return Success({
                    ...
                    "usage_percent": (total_backup_size / disk_usage.total * 100) if disk_usage.total > 0 else 0,
    ...
    async def _execute_backup(
        ...
        try:
            ...
            if policy.backup_type == BackupType.FULL:
                ...
                result = await self._create_incremental_backup(target, metadata, policy)
            elif policy.backup_type == BackupType.DIFFERENTIAL:
                result = await self._create_differential_backup(target, metadata, policy)
            elif policy.backup_type == BackupType.SNAPSHOT:
                result = await self._create_snapshot_backup(target, metadata, policy)
            else:
            ...
            if policy.verify_after_backup:
                ...
                if isinstance(verify_result, Failure):
                    operation.status = BackupStatus.FAILED
                    operation.error_message = f"Backup verification failed: {verify_result.error}"
                    return
    ...
    async def _execute_restore(
        ...
        try:
            ...
            if target_path.exists() and not operation.overwrite_existing:
                operation.status = BackupStatus.FAILED
                operation.error_message = f"Target path exists and overwrite is disabled"
                return
        ...
        try:
            ...
            with tarfile.open(backup_path, 'w:gz') as tar:
                ...
                for item in source_path.rglob('*'):
                    ...
                    # 포함 패턴 확인
                    if target.include_patterns and not self._should_include(
                        str(item), target.include_patterns
                    ):
                        continue
    ...
    async def _create_snapshot_backup(
        ...
        """스냅샷 백업 생성"""
        # 스냅샷은 전체 백업과 동일하게 처리 (추후 파일시스템 스냅샷 지원 추가 가능)
        return await self._create_full_backup(target, metadata, policy)
        ...
        while self._running:
            try:
                ...
                for policy in self.policies.values():
                    # Cron 표현식 파싱 및 실행 여부 확인 (간단한 구현)
                    # 실제로는 croniter 등의 라이브러리 사용 권장
            ...
            try:
                ...
                for policy in self.policies.values():
                    ...
                    for metadata in list(self.backup_history):
                        ...
                        if metadata.end_time and (current_time - metadata.end_time) > retention_delta:
                            await self.delete_backup(metadata.backup_id)
                
                # 저장소 용량 확인 및 정리
                if self.storage_config.auto_cleanup:
                    usage_result = await self.get_storage_usage()
                    if isinstance(usage_result, Success):
                        usage = usage_result.value
                        if usage['usage_percent'] > self.storage_config.cleanup_threshold_percent:
                            # 오래된 백업부터 삭제
                            ...
                            for metadata in sorted_backups[:5]:  # 최대 5개씩 삭제
                                await self.delete_backup(metadata.backup_id)
                                ...
                                if isinstance(usage_result, Success):
                                    usage = usage_result.value
                                    if usage['usage_percent'] <= self.storage_config.cleanup_threshold_percent * 0.9:
                                        break
    ...
    def _find_last_backup(self, target_id: str, policy_id: str) -> Optional[BackupMetadata]:
        """마지막 백업 찾기"""
        filtered = [
            m for m in self.backup_history
    ...
    def _find_last_full_backup(self, target_id: str, policy_id: str) -> Optional[BackupMetadata]:
        """마지막 전체 백업 찾기"""
        filtered = [
            m for m in self.backup_history
            if m.target_id == target_id and 
               m.policy_id == policy_id and
               m.backup_type == BackupType.FULL
        ]
