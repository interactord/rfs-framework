...
from typing import Dict, Any, Type, List, Optional, Protocol, Callable, Union
...
# 기존 코드와의 호환성을 위해 import
from .registry import ServiceScope, ServiceDefinition
...
class AnnotationMetadata:
    ...
    lazy: bool = False
    profile: Optional[str] = None  # 환경별 프로파일 (dev, prod, test)
    
    # 헥사고날 아키텍처 관련
    port_name: Optional[str] = None  # Adapter인 경우 구현하는 Port
    route: Optional[str] = None      # Controller인 경우 라우트 정보
...
def Adapter(port: str, name: str = None, scope: ComponentScope = ComponentScope.SINGLETON, 
           profile: str = None) -> Callable[[Type], Type]:
    """
...
def Component(name: str = None, scope: ComponentScope = ComponentScope.SINGLETON,
             dependencies: List[str] = None, lazy: bool = False,
             profile: str = None) -> Callable[[Type], Type]:
    """
    일반적인 컴포넌트 정의 어노테이션
...
def UseCase(name: str = None, dependencies: List[str] = None,
           scope: ComponentScope = ComponentScope.SINGLETON) -> Callable[[Type], Type]:
    """
...
def Controller(route: str = None, name: str = None,
...
# 기존 코드와의 호환성을 위한 별칭들
def Service(name: str = None, scope: ComponentScope = ComponentScope.SINGLETON,
           dependencies: List[str] = None, lazy: bool = False) -> Callable[[Type], Type]:
    """@Component의 별칭 - 서비스 레이어용"""
    return Component(name=name, scope=scope, dependencies=dependencies, lazy=lazy)


def Repository(name: str = None, scope: ComponentScope = ComponentScope.SINGLETON,
              dependencies: List[str] = None) -> Callable[[Type], Type]:
...
def is_controller(cls: Type) -> bool:
    ...
    metadata = get_annotation_metadata(cls)
    return metadata is not None and metadata.annotation_type == AnnotationType.CONTROLLER
...
def validate_hexagonal_architecture(classes: List[Type]) -> List[str]:
    ...
    for cls in classes:
        ...
        if metadata.annotation_type == AnnotationType.PORT:
            ports[metadata.name] = cls
            
        elif metadata.annotation_type == AnnotationType.ADAPTER:
            adapters[metadata.name] = metadata.port_name
