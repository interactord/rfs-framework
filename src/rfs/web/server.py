...
class RFSWebServer:
    ...
    def _determine_framework(self):
        """사용할 프레임워크 결정"""
        if self.config.framework == WebFramework.FASTAPI:
            if not FASTAPI_AVAILABLE:
                raise RuntimeError("FastAPI가 설치되지 않았습니다. pip install fastapi[all] 실행")
            self.framework = WebFramework.FASTAPI
        elif self.config.framework == WebFramework.FLASK:
            if not FLASK_AVAILABLE:
                raise RuntimeError("Flask가 설치되지 않았습니다. pip install flask flask-cors 실행")
