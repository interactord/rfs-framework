...
class SyncRequestHandler(RequestHandler):
    ...
    async def handle(self, request: Any, **kwargs) -> Any:
        ...
        try:
            ...
            # 메인 처리 (동기 함수를 스레드에서 실행)
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(
                None, 
                lambda: self.handler_func(request, **kwargs)
            )
...
class ErrorHandler:
    ...
    async def _default_error_handler(self, error: Exception, request: Any = None) -> Any:
        """기본 에러 처리"""
