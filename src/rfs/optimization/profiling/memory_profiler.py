...
class MemorySnapshot:
    ...
    traced_memory: Optional[Tuple[int, int]] = None  # (current, peak)
...
class MemoryMetrics:
    ...
    def detect_memory_trends(self, minutes: int = 5) -> Dict[str, str]:
        ...
        # 트렌드 분류
        if slope > 1000000:  # 1MB per interval
            return {"trend": "rapidly_increasing", "slope": slope}
        elif slope > 100000:  # 100KB per interval
...
class MemoryProfiler:
    ...
    def __init__(self, collection_interval: float = 2.0, enable_tracemalloc: bool = True):
        ...
        self.leak_detection_window = 50  # 스냅샷 개수
        self.leak_threshold_growth = 1.5  # 50% 증가시 누수로 의심
        ...
        try:
            ...
            try:
                ...
                if hasattr(gc, 'get_stats'):
                    gc_stats = {f"generation_{i}": stats for i, stats in enumerate(gc.get_stats())}
            except:
                pass
    ...
    async def _detect_memory_leaks(self):
        ...
        try:
            ...
            if growth_ratio > self.leak_detection_threshold_growth:
                ...
                confidence = min(1.0, (growth_ratio - 1.0) * 2)  # 간단한 신뢰도 계산
                ...
                self.metrics.detected_leaks.append(leak)
    ...
    async def _check_memory_alerts(self, snapshot: MemorySnapshot):
        ...
        try:
            ...
            if snapshot.swap_percent > 50.0:
                alerts.append(f"High swap usage: {snapshot.swap_percent:.1f}%")
            
            # 프로세스 메모리 급증
            if len(self.metrics.snapshots) > 1:
                prev_rss = self.metrics.snapshots[-2].rss
                current_rss = snapshot.rss
                if current_rss > prev_rss * 1.2:  # 20% 증가
                    alerts.append(f"Rapid memory increase: {(current_rss - prev_rss) / 1024 / 1024:.1f} MB")
    ...
    async def analyze_object_growth(self, minutes: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent[0].gc_counts and recent[-1].gc_counts:
                for i, (start, end) in enumerate(zip(recent[0].gc_counts, recent[-1].gc_counts)):
                    analysis["gc_collections"][f"generation_{i}"] = end - start
    ...
    def get_tracemalloc_statistics(self, limit: int = 10) -> Optional[List[Dict[str, Any]]]:
...
# 편의 함수들
def create_memory_profiler(collection_interval: float = 2.0, enable_tracemalloc: bool = True) -> MemoryProfiler:
    """메모리 프로파일러 생성"""
    return MemoryProfiler(collection_interval=collection_interval, enable_tracemalloc=enable_tracemalloc)
