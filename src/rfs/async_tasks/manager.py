...
from .executor import TaskExecutor, AsyncIOExecutor
    ...
    async def submit(
        ...
        **kwargs
        ...
        async with self._lock:
            ...
            self.task_instances[metadata.task_id] = task
            
            # Future 생성
            future = asyncio.Future()
            self.task_futures[metadata.task_id] = future
    ...
    async def _execute_task(self, task_id: str):
        ...
        try:
            ...
            await self._handle_task_failure(
                task_id,
                TaskTimeout(f"Task {task_id} timed out"),
                TaskStatus.TIMEOUT
            )
            
        except Exception as e:
            # 실패
            if metadata.retry_policy and metadata.retry_policy.should_retry(e, metadata.retry_count + 1):
                # 재시도
