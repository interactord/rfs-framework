...
import functools
import logging
from typing import Any, Callable, Optional, List, Union, Set, Dict
...
class User:
    ...
    def has_permission(self, permission: Union[Permission, str]) -> bool:
        ...
        # 역할 기반 권한 확인 (SUPER_ADMIN은 모든 권한)
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if not user.is_active:
                logger.warning(f"Inactive user {user.username} attempted to access {func.__name__}")
                raise AuthenticationError("User account is inactive")
            
            if check_verified and not user.is_verified:
                logger.warning(f"Unverified user {user.username} attempted to access {func.__name__}")
                raise AuthenticationError("Email verification required")
            
            if not allow_service_account and Role.SERVICE in user.roles:
                logger.warning(f"Service account {user.username} denied access to {func.__name__}")
...
def RequiresPermission(
    *required_permissions: Union[Permission, str],
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if require_all:
                ...
                if not has_any:
                    ...
                    raise AuthorizationError(f"One of permissions {perm_names} required")
...
def RequiresOwnership(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            # 관리자는 모든 리소스 접근 가능
            if allow_admin and (Role.ADMIN in user.roles or Role.SUPER_ADMIN in user.roles):
                return await func(*args, **kwargs)
            ...
            if custom_checker:
                ...
                if resource_id:
                    is_owner = await custom_checker(user, resource_id) if asyncio.iscoroutinefunction(custom_checker) else custom_checker(user, resource_id)
                    if not is_owner:
                        logger.warning(
                ...
                # 기본 소유권 확인 (kwargs에서 owner_field 확인)
...
class TokenManager:
    ...
    def __init__(
        ...
        algorithm: str = "HS256",
        access_token_expire: int = 3600,  # 1시간
        refresh_token_expire: int = 86400 * 7  # 7일
    ):
