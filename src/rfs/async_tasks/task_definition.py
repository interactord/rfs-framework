...
class TaskDefinition:
    ...
    def _validate_handler_signature(self) -> None:
        ...
        if not params:
            raise ValueError(f"Handler must accept TaskContext as first parameter: {self.name}")
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(self.handler):
                ...
                if timeout:
                    ...
                    result = await asyncio.to_thread(self.handler, context, *args, **kwargs)
    ...
    def _parse_cron(self, cron_expr: str, from_time: datetime) -> Optional[datetime]:
        ...
        if parts[0] == "0" and parts[1] == "0":
            next_day = from_time.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
            return next_day
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "schedule_interval": self.schedule_interval.total_seconds() if self.schedule_interval else None,
...
class TaskRegistry:
    ...
    def register(self, task_def: TaskDefinition) -> None:
        ...
        self._handlers[task_def.name] = task_def.handler
