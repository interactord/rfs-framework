...
class DatabaseConfig:
    ...
    def get_tortoise_config(self) -> Dict[str, Any]:
        ...
        config = {
            "connections": {
                "default": {
                    "engine": "tortoise.backends.asyncpg" if "postgresql" in self.url else "tortoise.backends.aiosqlite",
                    "credentials": {
                        "database": self.url.split("/")[-1] if "/" in self.url else self.url,
                        "host": "localhost",
                        "port": 5432,
                        "user": "postgres",
                        "password": "",
...
class ConnectionPool:
    ...
    async def initialize(self) -> Result[None, str]:
        ...
        try:
            ...
            logger.info(f"데이터베이스 연결 풀 초기화 완료: {self.config.database_type}")
            return Success(None)
...
class DatabaseManager(metaclass=SingletonMeta):
    ...
    async def add_database(self, name: str, config: DatabaseConfig) -> Result[None, str]:
        """데이터베이스 추가"""
        try:
