...
from .base import ServiceEndpoint, LoadBalancerType
...
class WeightedBalancer(LoadBalancer):
    ...
    def select(self, endpoints: List[ServiceEndpoint]) -> Optional[ServiceEndpoint]:
        ...
        for endpoint in endpoints:
            ...
            weighted_endpoints.extend([endpoint] * adjusted_weight)
    ...
    def report_success(self, endpoint: ServiceEndpoint):
        """성공 보고"""
        self.failures[endpoint] = max(0, self.failures.get(endpoint, 0) - 1)
...
class ServiceClient:
    ...
    async def _discover_services(self):
        ...
        if isinstance(result, Success):
            ...
            logger.info(f"Discovered {len(self.endpoints)} endpoints for {self.service_name}")
