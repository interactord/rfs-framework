...
from typing import Dict, Any, List, Optional, Type, Union, ClassVar
...
class SQLAlchemyModel(BaseModel, SQLAlchemy_Base):
    ...
    # 공통 필드
    id = Column(Integer, primary_key=True, autoincrement=True) if SQLALCHEMY_AVAILABLE else None
    created_at = Column(DateTime, default=datetime.utcnow) if SQLALCHEMY_AVAILABLE else None
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow) if SQLALCHEMY_AVAILABLE else None
...
def create_model(
    ...
    if base_class is None:
        ...
        if database and hasattr(database, 'config'):
            if database.config.orm_type.value in ['sqlalchemy', 'auto'] and SQLALCHEMY_AVAILABLE:
                base_class = SQLAlchemyModel
            elif database.config.orm_type.value in ['tortoise', 'auto'] and TORTOISE_AVAILABLE:
    ...
    # 모델 속성 생성
    attrs = {
        '__table_name__': table_name or name.lower(),
        '__fields__': fields
    }
    ...
    # Tortoise ORM 필드 추가
        ...
        for field_name, field_def in fields.items():
            if field_def.field_type == 'integer':
                ...
                field_obj = fields.CharField(max_length=field_def.max_length or 255, null=field_def.nullable)
            elif field_def.field_type == 'text':
                field_obj = fields.TextField(null=field_def.nullable)
            elif field_def.field_type == 'datetime':
                field_obj = fields.DatetimeField(auto_now_add=True if field_def.default else False)
            elif field_def.field_type == 'boolean':
