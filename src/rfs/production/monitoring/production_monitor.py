...
class MonitoringConfig:
    ...
    alert_cooldown_seconds: float = 300.0  # 5분 쿨다운
    max_metrics_history: int = 2880  # 24시간 (30초 간격)
...
class MetricsCollector:
    ...
    def register_custom_collector(self, name: str, collector: Callable[[], float]) -> None:
        """커스텀 메트릭 수집기 등록"""
        ...
        for name, collector in self.custom_collectors.items():
            try:
                ...
                if isinstance(value, (int, float)):
                    custom_metrics[name] = float(value)
            except Exception as e:
    ...
    def _determine_system_status(self, metrics_data: Dict[str, Any]) -> SystemStatus:
        ...
        # 크리티컬 상태 확인
        if (cpu >= thresholds.cpu_critical_percent or
            memory >= thresholds.memory_critical_percent or
            disk >= thresholds.disk_critical_percent):
            return SystemStatus.CRITICAL
        
        # 경고 상태 확인
        if (cpu >= thresholds.cpu_warning_percent or
            memory >= thresholds.memory_warning_percent or
            disk >= thresholds.disk_warning_percent):
    ...
    def _determine_service_health(self, metrics_data: Dict[str, Any]) -> ServiceHealth:
        ...
        # 성능 저하 확인
        if (response_time >= thresholds.response_time_critical_ms or
            error_rate >= thresholds.error_rate_warning_percent):
...
class AlertGenerator:
    ...
    def check_thresholds(self, metrics: ProductionMetrics) -> List[Dict[str, Any]]:
        ...
        # CPU 알림
        alerts.extend(self._check_cpu_thresholds(metrics, thresholds, current_time))
        
        # 메모리 알림
        alerts.extend(self._check_memory_thresholds(metrics, thresholds, current_time))
        
        # 디스크 알림
        alerts.extend(self._check_disk_thresholds(metrics, thresholds, current_time))
        
        # 응답 시간 알림
        alerts.extend(self._check_response_time_thresholds(metrics, thresholds, current_time))
        
        # 에러율 알림
        alerts.extend(self._check_error_rate_thresholds(metrics, thresholds, current_time))
        
        # 시스템 상태 알림
        alerts.extend(self._check_system_status_alerts(metrics, current_time))
    ...
    def _check_cpu_thresholds(self, metrics: ProductionMetrics, 
    ...
    def _check_memory_thresholds(self, metrics: ProductionMetrics,
                               thresholds: MonitoringThresholds,
                               current_time: datetime) -> List[Dict[str, Any]]:
        """메모리 임계값 확인"""
        alerts = []
    ...
    def _check_disk_thresholds(self, metrics: ProductionMetrics,
                             thresholds: MonitoringThresholds,
                             current_time: datetime) -> List[Dict[str, Any]]:
        """디스크 임계값 확인"""
        alerts = []
    ...
    def _check_response_time_thresholds(self, metrics: ProductionMetrics,
                                      thresholds: MonitoringThresholds,
                                      current_time: datetime) -> List[Dict[str, Any]]:
        """응답 시간 임계값 확인"""
        alerts = []
    ...
    def _check_error_rate_thresholds(self, metrics: ProductionMetrics,
                                   thresholds: MonitoringThresholds,
                                   current_time: datetime) -> List[Dict[str, Any]]:
        """에러율 임계값 확인"""
        alerts = []
    ...
    def _check_system_status_alerts(self, metrics: ProductionMetrics,
                                   current_time: datetime) -> List[Dict[str, Any]]:
        """시스템 상태 알림 확인"""
        alerts = []
...
class ProductionMonitor:
    ...
    async def initialize(self) -> Result[bool, str]:
        ...
        try:
            ...
            if initial_metrics.is_success():
                self.metrics_history.append(initial_metrics.unwrap())
    ...
    def register_custom_metric_collector(self, name: str, collector: Callable[[], float]) -> None:
        """커스텀 메트릭 수집기 등록"""
    ...
    async def get_health_report(self) -> Dict[str, Any]:
        ...
        performance_trends = self.get_performance_trends(1)  # 1시간
        ...
        return {
            ...
            'recommendations': self._generate_recommendations(current_metrics, recent_alerts)
        }
    
    def _calculate_health_score(self, metrics: ProductionMetrics, 
                               recent_alerts: List[Dict[str, Any]]) -> float:
        """헬스 점수 계산"""
        score = 100.0
        ...
        # 최근 알림 기반 점수 차감
        critical_alerts = [a for a in recent_alerts if a.get('severity') == AlertSeverity.CRITICAL]
        score -= len(critical_alerts) * 5
        
        return max(0.0, min(100.0, score))
    
    def _generate_recommendations(self, metrics: ProductionMetrics,
        ...
        if metrics.disk_usage_percent > 90:
            recommendations.append("High disk usage detected - clean up unused files or increase storage capacity")
        
        # 응답 시간 관련 추천
        if metrics.avg_response_time_ms > 2000:
            recommendations.append("High response time detected - optimize database queries or add caching")
        
        # 에러율 관련 추천
        if metrics.request_count > 0:
            error_rate = (metrics.error_count / metrics.request_count) * 100
            if error_rate > 1:
                recommendations.append(f"High error rate ({error_rate:.1f}%) - investigate error logs and fix issues")
...
def get_production_monitor(config: Optional[MonitoringConfig] = None) -> ProductionMonitor:
...
async def start_production_monitoring(config: Optional[MonitoringConfig] = None) -> Result[ProductionMonitor, str]:
    """프로덕션 모니터링 시작"""
