...
from .base import Task, CallableTask, TaskPriority
        ...
        if self.schedule_type == ScheduleType.CRON:
            ...
            if self.last_run:
                ...
                if self.last_run.month == 12:
                    next_run = self.last_run.replace(year=self.last_run.year + 1, month=1)
                else:
...
class TaskScheduler:
    ...
    async def schedule(
        ...
        logger.info(f"Task {scheduled_task.name} scheduled with ID {scheduled_task.task_id}")
                ...
                async with self._lock:
                    for task_id, scheduled_task in self.scheduled_tasks.items():
                        if scheduled_task.should_run():
                            ...
                            # 최대 실행 횟수 도달 시 비활성화
                            if scheduled_task.schedule.max_runs and \
                               scheduled_task.schedule.run_count >= scheduled_task.schedule.max_runs:
    ...
    async def _run_scheduled_task(self, scheduled_task: ScheduledTask):
        ...
        try:
            ...
            logger.info(
                f"Scheduled task {scheduled_task.name} submitted with ID {task_id} "
                f"(run {scheduled_task.schedule.run_count})"
            )
