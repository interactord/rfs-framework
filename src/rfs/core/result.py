...
from typing import Any, Callable, Generic, Iterator, Optional, TypeVar, List, Awaitable, Coroutine
...
async def async_try_except(func: Callable[[], T] | Callable[[], Awaitable[T]]) -> 'Result[T, Exception]':
...
def async_result_decorator(func: Callable[..., T | Awaitable[T]]) -> Callable[..., Awaitable['Result[T, Exception]']]:
...
class ResultAsync(Generic[T, E]):
    ...
    def map(self, func: Callable[[T], U] | Callable[[T], Awaitable[U]]) -> 'ResultAsync[U, E]':
        """비동기 값 변환"""
    ...
    def bind(self, func: Callable[[T], 'ResultAsync[U, E]'] | Callable[[T], 'Result[U, E]']) -> 'ResultAsync[U, E]':
        """비동기 결과 연결"""
...
async def sequence_async_v4(results: List['ResultAsync[T, E]']) -> 'ResultAsync[List[T], E]':
    ...
    async def sequence() -> 'Result[List[T], E]':
        ...
        resolved_results = await asyncio.gather(*result_awaitables, return_exceptions=False)
...
def traverse_either(items: List[T], func: Callable[[T], Either[U, E]]) -> Either[List[U], E]:
